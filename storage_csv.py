...
class StorageCsv(IStorage):
    ...
    def add_movie(self, title, user_notes):
        ...
        if response.status_code == requests.codes.ok:
            ...
            requested_parameters = ['Title', 'Year', 'Poster', 'imdbRating', 'imdbID']
            ...
            with open(self.filepath, "a") as file_obj:
                ...
                header = ["Title", "Year", "Poster", "imdbRating", "imdbID", "Notes"]
                writer = csv.DictWriter(file_obj, fieldnames=header)
                writer.writerow(requested_data)
    ...
    def update_movie(self, title, notes):
        ...
        df = pd.read_csv(self.filepath)
        df.loc[df["Title"] == title, "Notes"] = notes
        df.to_csv(self.filepath, index=False)

    def movie_serialisation(self):
        with open(self.filepath, "r") as file_obj:
            movies_data = csv.DictReader(file_obj)
    ...
    def generate_website(self):
        ...
        website_owner = f"{name_file_path}csv_database_showcase.html"
    ...
    def stats(self):
        with open(self.filepath, "r") as file_obj:
                ...
                for movie in range(len(all_movies)):
                    movie_ratings.append(float(all_movies[movie]["imdbRating"]))

                avg_rating = sum(movie_ratings) / len(movie_ratings)

                # helps the median formula
                n = 0 if len(all_movies) % 2 == 0 else 1
                # calculates the median
                median = (len(all_movies) + n) / 2
                ...
                for movie in range(len(all_movies)):
                    if top_value == float(all_movies[movie]["imdbRating"]):
                        top_values.append(all_movies[movie])
                ...
                bottom_values = []
                for movie in range(len(all_movies)):
                    if bottom_value == float(all_movies[movie]["imdbRating"]):
                        bottom_values.append(all_movies[movie])
    ...
    def random_movie(self):
        with open(self.filepath) as file:
            ...
            #print(all_movies)
            randomly_chosen = random.choice(all_movies)
            return f"{randomly_chosen['Title']}, Rating : {randomly_chosen['imdbRating']}, Year of realease : {randomly_chosen['Year']}"
        ...
        with open(self.filepath, "r") as file:
            ...
            for row in reader:
                if title.lower() in row["Title"].lower():
                    print("Movie found!")
                    print()
                    return f"Title : {row['Title']}, Rating : {row['imdbRating']}, Year of realease : {row['Year']}"
